package cn.shaviation.autotest.dialogs;
...
import cn.shaviation.autotest.text.JavaWordIterator;
			...
			if (fKeyListener == null) {
				fKeyListener = new KeyAdapter() {
					...
					public void keyPressed(KeyEvent e) {
						...
						getSubmissions();
						for (Iterator<Submission> iter = getSubmissions()
					...
					private List<Submission> getSubmissions() {
						...
						contextActivations = new IContextActivation[] {
								contextService
										.activateContext(IContextService.CONTEXT_ID_WINDOW),
								contextService
										.activateContext(TEXT_EDITOR_CONTEXT_ID) };
								...
								.add(new Submission(
									...
									public void execute() {
										...
										if (newCaret != BreakIterator.DONE) {
											Point selection = fNavigable
													.getSelection();
											if (caretPosition == selection.y)
												fNavigable.setSelection(
														selection.x, newCaret);
											else
												fNavigable.setSelection(
														selection.y, newCaret);
										}
								...
								.add(new Submission(
									...
									public void execute() {
										...
										int newCaret = fIterator
												.following(caretPosition);
										if (newCaret != BreakIterator.DONE)
											fNavigable.setSelection(newCaret,
													newCaret);
										fIterator.setText(EMPTY_TEXT);
									}
								});
						...
						fSubmissions
								.add(new Submission(
									...
									public void execute() {
										...
										fNavigable.setText(text.substring(0,
												start) + text.substring(end));
