...
public class AntPathMatcher implements PathMatcher {
	...
	protected boolean doMatch(String pattern, String path, boolean fullMatch) {
		...
		List<String> pattDirs = Strings.split(pattern, this.pathSeparator);
		List<String> pathDirs = Strings.split(path, this.pathSeparator);

		int pattIdxStart = 0;
		int pattIdxEnd = pattDirs.size() - 1;
		int pathIdxStart = 0;
		int pathIdxEnd = pathDirs.size() - 1;

		// Match all elements up to the first **
		while (pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
			String patDir = pattDirs.get(pattIdxStart);
			if ("**".equals(patDir)) {
				break;
			}
			if (!matchStrings(patDir, pathDirs.get(pathIdxStart))) {
				return false;
			}
			pattIdxStart++;
			pathIdxStart++;
		...
		if (pathIdxStart > pathIdxEnd) {
			...
			if (pattIdxStart == pattIdxEnd && pattDirs.get(pattIdxStart).equals("*") &&
					path.endsWith(this.pathSeparator)) {
				return true;
			}
			for (int i = pattIdxStart; i <= pattIdxEnd; i++) {
				if (!pattDirs.get(i).equals("**")) {
					return false;
				}
			}
			return true;
		...
		else if (!fullMatch && "**".equals(pattDirs.get(pattIdxStart))) {
		...
		// up to last '**'
		while (pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
			String patDir = pattDirs.get(pattIdxEnd);
			if (patDir.equals("**")) {
				break;
			}
			if (!matchStrings(patDir, pathDirs.get(pathIdxEnd))) {
				return false;
			}
			pattIdxEnd--;
			pathIdxEnd--;
			...
			strLoop:
			    for (int i = 0; i <= strLength - patLength; i++) {
				    for (int j = 0; j < patLength; j++) {
					    String subPat = (String) pattDirs.get(pattIdxStart + j + 1);
					    String subStr = (String) pathDirs.get(pathIdxStart + i + j);
	...
	public String extractPathWithinPattern(String pattern, String path) {
		List<String> patternParts = Strings.split(pattern, this.pathSeparator);
		List<String> pathParts = Strings.split(path, this.pathSeparator);
		...
		for (int i = 0; i < patternParts.size(); i++) {
			...
			if ((patternPart.indexOf('*') > -1 || patternPart.indexOf('?') > -1) && pathParts.size() >= i + 1) {
				...
				buffer.append(pathParts.get(i));
				puts++;
			}
		}

		// Append any trailing path parts.
		for (int i = patternParts.size(); i < pathParts.size(); i++) {
			if (puts > 0 || i > 0) {
				buffer.append(this.pathSeparator);
			}
			buffer.append(pathParts.get(i));
